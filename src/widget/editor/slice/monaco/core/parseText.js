...
export const parseText = (text) => {
  ...
  const connectionHandler = () => {
    ...
    if (cur.word === ',' && (prev.word === '{' || prev.word === ',')) {
      errors.push({
        message: MSG.MONACO.COMMA_AFTER_LABEL,
    ...
    if (cur.word !== ',' && (prev.word === '{' || prev.word === ',')) {
      ...
      if (curNode.label === cur.word) {
        errors.push({
          message: MSG.MONACO.SELF_REFERENCE,
          wordRange: cur.wordRange,
        })
      }
      ...
      if (duplication) {
        errors.push(
          {
            message: MSG.MONACO.DUPLICATE_REFERENCE,
            wordRange: cur.wordRange,
          },
          {
            message: MSG.MONACO.DUPLICATE_REFERENCE,
    ...
    // 노드 커넥션 라벨 선언 이후의 옵션들
      ...
      if (cur.word === 'solid' || cur.word === 'dashed') {
        if (lastConnection.style) {
          ...
          errors.push(
            {
              message: MSG.MONACO.DUPLICATE_STYLE,
              wordRange: cur.wordRange,
            },
            {
              message: MSG.MONACO.DUPLICATE_STYLE,
      ...
      // 색상 옵션 처리
        ...
        if (lastConnection.color) {
          errors.push(
            {
              message: MSG.MONACO.DUPLICATE_COLOR,
              wordRange: cur.wordRange,
            },
            {
              message: MSG.MONACO.DUPLICATE_COLOR,
      ...
      // 화살표 옵션
        ...
        if (lastConnection.arrow) {
          errors.push(
            {
              message: MSG.MONACO.DUPLICATE_ARROW,
              wordRange: cur.wordRange,
            },
            {
              message: MSG.MONACO.DUPLICATE_ARROW,
        ...
        // 해당되는 옵션이 아닐경우 에러
        errors.push({
          message: MSG.MONACO.INVALID_CONN_OPTION,
          wordRange: cur.wordRange,
        })
        return
      }
  ...
  const wordHandler = () => {
    ...
    if (depth < 2 && (!next || next.word !== '{')) {
      errors.push({
        message: MSG.MONACO.MISSING_OPEN_BRACE,
        wordRange: cur.wordRange,
      })
      return
    }
  ...
  const openBraceHandler = () => {
    ...
    if (depth === 1) {
      ...
      if (prev === undefined || prev.word === '{' || prev.word === '}') {
        errors.push({
          message: MSG.MONACO.MISSING_NODE_NAME,
          wordRange: cur.wordRange,
        })
        return
      }
      ...
      if (duplication) {
        errors.push(
          {
            message: MSG.MONACO.DUPLICATE_NODE_NAME,
      ...
      if (prev === undefined || prev.word === '{' || prev.word === '}') {
        errors.push({
          message: MSG.MONACO.MISSING_KEYWORD,
          wordRange: cur.wordRange,
        })
        return
      }
      ...
      if (duplication) {
        ...
        errors.push(
          {
            message: MSG.MONACO.DUPLICATE_KEYWORD,
            wordRange: prev.wordRange,
          },
          {
            message: MSG.MONACO.DUPLICATE_KEYWORD,
      ...
      // 해당되는 keyword가 아니면 에러
        ...
        errors.push({
          message: MSG.MONACO.INVALID_KEYWORD,
          wordRange: prev.wordRange,
        })
        return
      }
      ...
      if (curKeyword !== 'content') {
        errors.push({
          message: MSG.MONACO.INVALID_BRACE_LOCATION,
          wordRange: prev.wordRange,
        })
        return
      }
  ...
  const closeBraceHandler = () => {
    ...
    if (braces.length === 0) {
      errors.push({
        message: MSG.MONACO.UNMATCHED_CLOSE_BRACE,
        wordRange: cur.wordRange,
      })
      return
    }
      ...
      if (node.connection) {
        node.connection.data = node.connection.data.filter((connection) => {
          if (!labels.has(connection.target)) {
            ...
            errors.push({
              message: MSG.MONACO.INVALID_REFERENCE,
